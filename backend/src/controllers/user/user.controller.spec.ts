...
describe('UserController', () => {
  ...
  describe('create', () => {
    it('should call create method', async () => {
      ...
      expect(mockService.create).toHaveBeenCalledWith({username: 'user', password: 'pass'});
    ...
    it('should raise an error when create fails', async () => {
      ...
      await expect(controller.create({username:'user', password:'pass'})).rejects.toThrow(Error);
      expect(mockService.create).toHaveBeenCalledWith({username:'user', password:'pass'});
      expect(mockService.create).toHaveBeenCalledTimes(1);
    });
  });
