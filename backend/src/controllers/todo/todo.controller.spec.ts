...
describe('TodoController', () => {
  ...
  describe('createTodo', () => {
    it('should call createTodo successfully', async () => {
      ...
      expect(mockTodoService.createTodo).toHaveBeenCalledWith(1, 1, {
        listId: 1,
        title: 'todo',
        detail: 'detail',
      });
  ...
  describe('updateTodo', () => {
    it('should call updateTodo successfully', async () => {
      ...
      await controller.updateTodo({ user: { userId: 1 } }, 1, {
        listId: 1,
        title: 'todo',
        detail: 'detail',
        isDone: true,
      ...
      expect(mockTodoService.updateTodo).toHaveBeenCalledWith(1, 1, {
        id: 1,
        title: 'todo',
        detail: 'detail',
        isDone: true,
    ...
    it('should fail when updateTodo throws an error', async () => {
      ...
      await expect(
        controller.updateTodo({ user: { userId: 1 } }, 1, {
          listId: 1,
          title: 'todo',
          detail: 'detail',
          isDone: true,
  ...
  describe('deleteTodo', () => {
    ...
    it('should fail when deleteTodo throws an error', async () => {
      ...
      await expect(
        controller.deleteTodo({ user: { userId: 1 } }, 1, 1),
      ).rejects.toThrow(Error);
      expect(mockTodoService.deleteTodo).toHaveBeenCalledTimes(1);
      expect(mockTodoService.deleteTodo).toHaveBeenCalledWith(1, 1, 1);
    });
