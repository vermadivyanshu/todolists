...
describe('ListController', () => {
  ...
  describe('create', () => {
    ...
    it('should create list when the user is authenticated', async () => {
      ...
      expect(mockTodoService.createList).toHaveBeenCalledWith(1, {
        name: 'list',
      });
    });
    ...
    it('should throw an error when service fails', async () => {
      ...
      await expect(
        controller.createList({ user: { userId: 1 } }, { name: 'list' }),
  ...
  describe('findAllListByUserId', () => {
    it('should find all lists for the user', async () => {
      ...
      jest.spyOn(mockAuthGuard, 'canActivate').mockResolvedValue(true);
      jest
        .spyOn(mockTodoService, 'findAllListByUserId')
    ...
    it('should throw an error when service fails', async () => {
      ...
      await expect(
        controller.findAllByUserId({ user: { userId: 1 } }),
      ).rejects.toThrow(Error);
      expect(mockTodoService.findAllListByUserId).toHaveBeenCalledTimes(1);
      expect(mockTodoService.findAllListByUserId).toHaveBeenCalledWith(1);
    });
  ...
  describe('findByUserIdAndListId', () => {
    it('should find list for the user and list id', async () => {
      ...
      jest.spyOn(mockAuthGuard, 'canActivate').mockResolvedValue(true);
      jest
        .spyOn(mockTodoService, 'findListByUserIdAndListId')
        .mockResolvedValue(list);
      await controller.findByUserIdAndListId({ user: { userId: 1 } }, 1);
      expect(mockTodoService.findListByUserIdAndListId).toHaveBeenCalledTimes(
        1,
      );
      expect(mockTodoService.findListByUserIdAndListId).toHaveBeenCalledWith(
        1,
        1,
      );
    });
    ...
    it('should throw an error when service fails', async () => {
      ...
      await expect(
        controller.findByUserIdAndListId({ user: { userId: 1 } }, 1),
      ).rejects.toThrow(Error);
      expect(mockTodoService.findListByUserIdAndListId).toHaveBeenCalledTimes(
        1,
      );
  ...
  describe('deleteListByListIdAndUserId', () => {
    it('should find list for the user and list id', async () => {
      ...
      jest.spyOn(mockAuthGuard, 'canActivate').mockResolvedValue(true);
      jest
        .spyOn(mockTodoService, 'deleteListByUserIdAndListId')
        .mockResolvedValue(list);
      await controller.deleteListByListIdAndUserId({ user: { userId: 1 } }, 1);
      expect(mockTodoService.deleteListByUserIdAndListId).toHaveBeenCalledTimes(
        1,
      );
      expect(mockTodoService.deleteListByUserIdAndListId).toHaveBeenCalledWith(
        1,
        1,
      );
    });
    ...
    it('should throw an error when service fails', async () => {
      ...
      await expect(
        controller.deleteListByListIdAndUserId({ user: { userId: 1 } }, 1),
      ).rejects.toThrow(Error);
      expect(mockTodoService.deleteListByUserIdAndListId).toHaveBeenCalledTimes(
        1,
      );
  ...
  describe('updateList', () => {
    it('should find list for the user and list id', async () => {
      ...
      jest.spyOn(mockAuthGuard, 'canActivate').mockResolvedValue(true);
      jest
        .spyOn(mockTodoService, 'updateListByUserIdAndListId')
        .mockResolvedValue(list);
      await controller.updateList({ user: { userId: 1 } }, { name: 'test' }, 1);
      expect(mockTodoService.updateListByUserIdAndListId).toHaveBeenCalledTimes(
        1,
      );
      expect(mockTodoService.updateListByUserIdAndListId).toHaveBeenCalledWith(
        1,
        1,
        'test',
      );
    ...
    it('should throw an error when service fails', async () => {
      ...
      await expect(
        controller.updateList({ user: { userId: 1 } }, { name: 'test' }, 1),
      ).rejects.toThrow(Error);
      expect(mockTodoService.updateListByUserIdAndListId).toHaveBeenCalledTimes(
        1,
      );
