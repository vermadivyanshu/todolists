...
describe('TodoService', () => {
  ...
  describe('createList', () => {
    it('should create a list', async () => {
      ...
      const [listEntities, count] = await listRepository.findAndCountBy({
        user: { id: user.id },
      });
      expect(count).toBe(1);
  ...
  describe('findAllListByUserId', () => {
    it('should find all lists for user', async () => {
      ...
      const list2 = await listRepository.save({
        name: 'list2',
        user: { id: user.id },
      });
      delete list.user;
  ...
  describe('findListByUserIdAndListId', () => {
    it('should find list that belongs to the user', async () => {
      ...
      const expectedResponse = await service.findListByUserIdAndListId(
        user.id,
        list.id,
      );
      delete todo.list;
    ...
    it('should return null when the list and user are not linked', async () => {
      ...
      await expect(
        service.findListByUserIdAndListId(user.id, list.id),
  ...
  describe('updateListByUserIdAndListId', () => {
    it('should update the list name', async () => {
      ...
      const expectedResponse = await service.updateListByUserIdAndListId(
        user.id,
        list.id,
        'list-update',
      );
    ...
    it('should throw error when name is an empty string', async () => {
      ...
      await expect(
        service.updateListByUserIdAndListId(user.id, list.id, ''),
      ).rejects.toThrow('list name must be present');
    });
    ...
    it('should throw an error when list does not exist', async () => {
      ...
      await expect(
        service.updateListByUserIdAndListId(user.id, list.id, 'update'),
      ).rejects.toThrow('list does not exist');
    });
  });
  ...
  describe('deleteListByUserIdAndListId', () => {
    ...
    it('should throw an error when list is not found', async () => {
      ...
      await expect(
        service.deleteListByUserIdAndListId(user.id, list.id),
      ).rejects.toThrow('list does not exist');
    });
  });
  ...
  describe('createTodo', () => {
    it('should create a todo', async () => {
      ...
      const expectedResponse = await service.createTodo(user.id, list.id, {
        title: ' test ',
        detail: ' test ',
      });
      const [entities, count] = await todoRepository.findAndCountBy({
        list: { id: list.id },
      });
      expect(count).toBe(1);
    ...
    it('should throw an error when title is empty', async () => {
      ...
      await expect(
        service.createTodo(user.id, list.id, { title: ' ', detail: ' ' }),
      ).rejects.toThrow('title or detail cannot be null');
    });
    ...
    it('should throw an error when list is not present', async () => {
      ...
      await expect(
        service.createTodo(user.id, list.id, { title: 'test', detail: 'test' }),
      ).rejects.toThrow('list does not exist');
    });
  });
  ...
  describe('findOneTodo', () => {
    ...
    it('should find todo', async () => {
      const expectedResponse = await service.findOneTodo(
        user.id,
        list.id,
        todo.id,
      );
    ...
    it('should throw an error when list does not exist', async () => {
      ...
      await expect(
        service.findOneTodo(user1.id, list.id, todo.id),
      ).rejects.toThrow('list does not exist');
    });

    it('should throw an error when todo is invalid does not exist', async () => {
      await expect(service.findOneTodo(user.id, list.id, -1)).rejects.toThrow(
        'todo does not exist',
      );
    });
  });
  ...
  describe('deleteTodo', () => {
    ...
    it('should throw an error when todo.id is invalid', async () => {
      ...
      await expect(service.deleteTodo(user.id, list.id, -1)).rejects.toThrow(
        'todo does not exist',
      );
    });
  });
  ...
  describe('updateTodo', () => {
    ...
    it('should update todo when title and detail are present', async () => {
      const expectedResponse = await service.updateTodo(user.id, list.id, {
        id: todo.id,
        title: ' update ',
        detail: ' detail ',
        isDone: true,
    ...
    it('should throw an error when title or detail are empty', async () => {
      await expect(
        service.updateTodo(user.id, list.id, {
          id: todo.id,
          title: ' ',
          detail: ' ',
          isDone: true,
    ...
    it('should throw an error when todo is not found', async () => {
      ...
      ).rejects.toThrow('todo does not exist');
    });
  });
});
